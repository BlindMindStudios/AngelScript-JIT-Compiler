...
int asCJITCompiler::CompileFunction(asIScriptFunction *function, asJITFunction *output) {
	...
	*output = activePage->getFunctionPointer<asJITFunction>();
	...
	//32-bit registers
	Register eax(cpu,EAX), ebx(cpu,EBX), ecx(cpu,ECX), edx(cpu,EDX), esp(cpu,ESP), ebp(cpu,EBP), esi(cpu,ESI), edi(cpu,EDI);
	//8-bit registers
	Register al(cpu,EAX,8), bl(cpu,EBX,8), cl(cpu,ECX,8), dl(cpu,EDX,8);
	//Pointer-sized registers
	Register pax(cpu,EAX,pBits), pbx(cpu,EBX,pBits), pcx(cpu,ECX,pBits), pdx(cpu,EDX,pBits);
	...
	eax = *esp+cpu.stackDepth; //Register pointer
	edi = *eax+offsetof(asSVMRegisters,stackFramePointer); //VM Frame pointer
	esi = *eax+offsetof(asSVMRegisters,stackPointer); //VM Stack pointer
	ebx = *eax+offsetof(asSVMRegisters,valueRegister); //VM Temporary
	ebp = *eax+offsetof(asSVMRegisters,valueRegister)+4; //VM Temporary (upper half)
	eax = *esp+cpu.stackDepth+cpu.pushSize(); //Entry jump pointer
	//}
	...
	eax = *esp+cpu.stackDepth; //Register pointer
	*eax = edx; //Set the bytecode pointer based on our exit
	*eax+offsetof(asSVMRegisters,stackFramePointer) = edi; //Return the frame pointer
	*eax+offsetof(asSVMRegisters,stackPointer) = esi; //Return the stack pointer
	*eax+offsetof(asSVMRegisters,valueRegister) = ebx; //Return the temporary
	*eax+offsetof(asSVMRegisters,valueRegister)+4 = ebp; //  ...and the other half
			...
			if(pThirdOp < end) {
				...
				case asBC_CpyVtoV4:
					if(nextOp == asBC_iTOf && offset(pOp,0) == offset(pNextOp,0)) {
						...
						fpu.store_double(*edi-offset(pNextOp,0));
					}
				}
			}
...
#define pushPrep(use) \
	...
	reservedPushBytes -= use;
		...
		case asBC_PSF:
			...
			as<void*>(*esi + reservedPushBytes) = eax;
			break;
		case asBC_PshG4:
			pushPrep(sizeof(asDWORD));
		...
		case asBC_PshVPtr:
			pushPrep(sizeof(void*));
			eax = as<void*>(*edi-offset0);
			as<void*>(*esi + reservedPushBytes) = eax;
			break;
		case asBC_PshRPtr: 
			pushPrep(sizeof(void*));
			*esi + reservedPushBytes = ebx;
			break;
		case asBC_PshNull:
			pushPrep(sizeof(void*));
			eax ^= eax;
		...
		case asBC_OBJTYPE:
			pushPrep(sizeof(void*));
			*esi + reservedPushBytes = (void*)asBC_PTRARG(pOp);
			break;
		case asBC_TYPEID:
			pushPrep(sizeof(asDWORD));
			*esi + reservedPushBytes = asBC_DWORDARG(pOp);
		...
		case asBC_VAR:
			pushPrep(sizeof(void*));
			*esi + reservedPushBytes = (size_t)asBC_SWORDARG0(pOp);
			break;

		//Now the normally-ordered ops
		case asBC_SwapPtr:
			eax = as<void*>(*esi);
			eax.swap(as<void*>(*esi+sizeof(void*)));
			as<void*>(*esi) = eax;
			break;
		case asBC_NOT:
			{
				ecx.copy_address(*edi-offset0);
		...
		case asBC_CALL:
			{
				...
				*edx + offsetof(asSVMRegisters,programPointer) = pOp+2;
				*edx + offsetof(asSVMRegisters,stackPointer) = esi;
				edx = as<void*>(*edx + offsetof(asSVMRegisters,ctx));
		...
		case asBC_RET:
			{
				...
				ecx = as<void*>(*ecx + offsetof(asSVMRegisters,ctx));
				cpu.call_stdcall((void*)popStackIfNotEmpty,"r",&ecx);
				...
				edx = *esp+cpu.stackDepth; //Register pointer
				*edx+offsetof(asSVMRegisters,valueRegister) = ebx; //VM Temporary
				*edx+offsetof(asSVMRegisters,valueRegister)+4 = ebp; //VM Temporary (upper half)
				*edx+offsetof(asSVMRegisters,stackPointer) += asBC_WORDARG0(pOp)*sizeof(asDWORD);
				cpu.pop(ebp);
				cpu.pop(ebx);
				cpu.pop(edi);
				cpu.pop(esi);
		...
		case asBC_JMPP:
			if((flags & JIT_NO_SWITCHES) == 0) {
				...
				pax = jumpTable + ((pOp + 1) - start);

				pdx.copy_expanding(as<int>(*edi - offset0));
				pdx += pdx;

				pcx = *pax + pdx*sizeof(void*);
		...
		case asBC_STR:
			{
				...
				*esi + sizeof(asDWORD) = (void*)str.AddressOf();
				*esi = (asDWORD)str.GetLength();
			} break;
		case asBC_CALLSYS:
			{
				check_space(256);
		...
		case asBC_CALLBND:
			{
				...
				*edx + offsetof(asSVMRegisters,programPointer) = pOp+2;
				*edx + offsetof(asSVMRegisters,stackPointer) = esi;
				edx = *edx + offsetof(asSVMRegisters,ctx);
		...
		case asBC_SUSPEND:
			if(flags & JIT_NO_SUSPEND) {
				...
				eax = *esp + cpu.stackDepth;

				//Check if we should suspend
				cl = *eax+offsetof(asSVMRegisters,doProcessSuspend);
				cl &= cl;
		...
		case asBC_ALLOC:
			{
				...
				if(objType->flags & asOBJ_SCRIPT_OBJECT) {
					...
					if( func ) {
						cpu.push(eax); cpu.stackDepth += sizeof(void*);						
						auto pFunc = (asCScriptFunction*)function->GetEngine()->GetFunctionById(func);

						sysCall.callSystemFunction(pFunc, &eax);
		...
		case asBC_FREE:
			...
			if(!(objType->flags & asOBJ_REF) || !(objType->flags & asOBJ_NOCOUNT)) { //Only do FREE on non-reference types, or reference types without fake reference counting
				...
				if((flags & JIT_ALLOC_SIMPLE) == 0) {
					...
					ecx = *esp+cpu.stackDepth;
					*ecx + offsetof(asSVMRegisters,programPointer) = pOp;
		...
		case asBC_GETOBJ:
			{
				eax.copy_address(*esi+offset0);

				edx = as<asDWORD>(*eax); //-Offset
				-edx;
				ecx.copy_address(*edi+edx*4);

				as<void*>(*eax).direct_copy( as<void*>(*ecx), edx);
				*ecx = nullptr;
			} break;
		case asBC_REFCPY:
			{
			...
			if(objType->flags & asOBJ_NOCOUNT) {
				...
				*pax = pcx;
				...
				ecx = *esp+cpu.stackDepth;
				*ecx + offsetof(asSVMRegisters,programPointer) = pOp;
				as<void*>(*ecx + offsetof(asSVMRegisters,stackPointer)) = esi;
		...
		case asBC_GETOBJREF:
			eax.copy_address(*esi + (asBC_WORDARG0(pOp)*sizeof(asDWORD)));

			edx = *eax; //-Offset
			-edx;

			as<void*>(*eax).direct_copy( as<void*>(*edi+edx*4), edx);
			break;
		case asBC_GETREF:
			eax.copy_address(*esi + (asBC_WORDARG0(pOp)*sizeof(asDWORD)));
		...
		case asBC_MODf:
			...
			cpu.call_stdcall((void*)fmod_wrapper_f,"rr",&ecx,&edx);
			fpu.store_float(*edi-offset0);
			break;
		case asBC_ADDd:
			fpu.load_double(*edi-offset1);
		...
		case asBC_MODd:
			ecx.copy_address(*edi-offset1);
			edx.copy_address(*edi-offset2);
			cpu.call_stdcall((void*)fmod_wrapper,"rr",&ecx,&edx);
			fpu.store_double(*edi-offset0);
			break;
		case asBC_ADDIi:
		...
		case asBC_CALLINTF:
			{
				...
				*edx + offsetof(asSVMRegisters,stackPointer) = esi;
				...
				MemAddress ctxPtr( as<void*>(*edx + offsetof(asSVMRegisters,ctx)) );
		...
		case asBC_Cast: //Can't handle casts (script call)
			{
				...
				ecx = as<void*>(*esp+cpu.stackDepth);
				as<void*>(*ecx + offsetof(asSVMRegisters,objectRegister)) = eax;
...
#define cast(f,t) {\
	...
	if(sizeof(f) != sizeof(t))\
		{ ecx.copy_address(*edi-offset1); cpu.call_stdcall(func,"rr",&ecx,&eax); }\
	else\
		cpu.call_stdcall(func,"rr",&eax,&eax);\
	}
#else
#define cast(f,t) {\
	void* func = (void*)(void (*)(f*,t*))(directConvert<f,t>);\
	eax.copy_address(*edi-offset0);\
	if(sizeof(f) != sizeof(t))\
		{ ecx.copy_address(*edi-offset1); cpu.call_cdecl(func,"rr",&ecx,&eax); }\
	else\
		cpu.call_cdecl(func,"rr",&eax,&eax);\
	}
#endif
		...
		case asBC_uTOi64:
			cast(unsigned long, long long) break;
		case asBC_iTOi64:
		...
		case asBC_DIVi64:
			...
			edx.copy_address(*edi-offset2);
			eax.copy_address(*edi-offset0);
			cpu.call_stdcall((void*)i64_div,"rrr",&ecx,&edx,&eax);
			break;
		case asBC_MODi64:
			ecx.copy_address(*edi-offset1);
			edx.copy_address(*edi-offset2);
			eax.copy_address(*edi-offset0);
			cpu.call_stdcall((void*)i64_mod,"rrr",&ecx,&edx,&eax);
			break;
		case asBC_BAND64:
			ecx = *edi-offset1;
			edx = *edi-offset1+4;
		...
		case asBC_CMPi64:
			ecx.copy_address(*edi-offset0);
			edx.copy_address(*edi-offset1);
			cpu.call_stdcall((void*)cmp_int64,"rr",&ecx,&edx);
			ebx = eax;
			break;
		case asBC_CMPu64:
			ecx.copy_address(*edi-offset0);
			edx.copy_address(*edi-offset1);
			cpu.call_stdcall((void*)cmp_uint64,"rr",&ecx,&edx);
			ebx = eax;
			break;
		case asBC_ChkNullS:
			{
		...
		case asBC_CallPtr:
			{
				...
				edx = *esp+cpu.stackDepth;
				*edx + offsetof(asSVMRegisters,programPointer) = pOp+1;
				*edx + offsetof(asSVMRegisters,stackPointer) = esi;
				edx = as<void*>(*edx + offsetof(asSVMRegisters,ctx));

				cpu.call_stdcall((void*)callScriptFunction,"rr",&edx,&ecx);
				ReturnFromScriptCall();
			} break;
		...
		case asBC_LoadRObjR:
			...
			ebx += asBC_SWORDARG1(pOp);
			break;
		case asBC_LoadVObjR:
			ebx.copy_address(*edi+(asBC_SWORDARG1(pOp) - offset0));
			break;
		default:
...
void SystemCall::call_entry(asSSystemFunctionInterface* func, asCScriptFunction* sFunc) {
	Register eax(cpu,EAX), edx(cpu,EDX), esp(cpu,ESP), esi(cpu,ESI,sizeof(void*) * 8);

	if(!(flags & JIT_SYSCALL_FPU_NORESET))
		fpu.init();

	edx = as<void*>(*esp + cpu.stackDepth);
	*edx + offsetof(asSVMRegisters,programPointer) = pOp;
	as<void*>(*edx + offsetof(asSVMRegisters,stackPointer)) = esi;

	if(!(flags & JIT_SYSCALL_NO_ERRORS)) {
		edx = as<void*>(*edx + offsetof(asSVMRegisters,ctx));
		edx += offsetof(asCContext,callingSystemFunction); //&callingSystemFunction
		as<void*>(*edx) = sFunc;

		cpu.push(edx); cpu.stackDepth += cpu.pushSize();
	}
}
...
void SystemCall::call_exit(asSSystemFunctionInterface* func) {
	...
	if(!(flags & JIT_SYSCALL_NO_ERRORS)) {
		cpu.pop(eax); //IsSystem*
		as<void*>(*eax) = 0u;

		cpu.stackDepth -= cpu.pushSize();

		//Check if we should suspend
		eax = as<void*>(*esp+cpu.stackDepth);
		cl = *eax+offsetof(asSVMRegisters,doProcessSuspend);
		cl &= cl;
		returnHandler(NotZero);
	}
}
...
void SystemCall::call_viaAS(asCScriptFunction* func, Register* objPointer) {
	...
	Register eax(cpu,EAX), ebx(cpu,EBX), ecx(cpu,ECX), edx(cpu,EDX), esp(cpu,ESP), ebp(cpu,EBP), esi(cpu,ESI), edi(cpu,EDI);
	Register pax(cpu,EAX,sizeof(void*) * 8), pcx(cpu,ECX,sizeof(void*) * 8);
	Register cl(cpu,ECX,8);
	...
	if(func->sysFuncIntf->hostReturnSize >= 1 && !func->sysFuncIntf->hostReturnInMemory
		&& !(func->returnType.IsObject() && !func->returnType.IsReference()) ) {

		ecx = *esp+cpu.stackDepth;
		ebx = *ecx + offsetof(asSVMRegisters,valueRegister);

		if(func->sysFuncIntf->hostReturnSize >= 2)
